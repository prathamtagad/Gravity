...
const Navigation: React.FC = () => {
  ...
  const navLinks = [
    { 
      ...
      label: 'Map', 
      icon: (active: boolean) => (
        <svg className={`w-6 h-6 transition-all ${active ? 'fill-neutral-900' : 'stroke-neutral-400 fill-none'}`} viewBox="0 0 24 24" strokeWidth={active ? 0 : 2}>
    ...
    { 
      ...
      label: 'Ranks', 
      icon: (active: boolean) => (
        <svg className={`w-6 h-6 transition-all ${active ? 'fill-neutral-900 stroke-neutral-900' : 'stroke-neutral-400 fill-none'}`} viewBox="0 0 24 24" strokeWidth={2}>
    ...
    { 
      ...
      label: 'Boost', 
      icon: (active: boolean) => (
        <div className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all ${active ? 'bg-neutral-900 shadow-xl scale-110' : 'bg-neutral-100 hover:bg-neutral-200'}`}>
    ...
    { 
      ...
      label: 'Me', 
      icon: (active: boolean) => (
        <div className={`w-6 h-6 rounded-lg overflow-hidden border-2 transition-all ${active ? 'border-neutral-900 scale-110' : 'border-transparent'}`}>
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest leading-none mt-1">Study Companion</span>
            </div>
          </Link>

          <div className="flex items-center gap-2 bg-neutral-50 p-1.5 rounded-[24px] border border-neutral-100">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={handleSignOut} className="p-3 text-neutral-300 hover:text-neutral-900 transition-colors">
      ...
      {!isMobileChat && (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[1100] h-16 bg-white/95 backdrop-blur-3xl border-t border-neutral-100 flex items-center justify-around px-2 pb-safe font-sans shadow-[0_-1px_10px_rgba(0,0,0,0.02)]">
            {navLinks.map((link) => (
            <Link
                key={link.path}
                to={link.path}
            ...
            >
                ...
                <span className={`text-[8px] font-bold uppercase tracking-widest mt-1.5 transition-all ${isActive(link.path) ? 'text-neutral-900 opacity-100' : 'text-neutral-300 opacity-0'}`}>
                {link.label}
                </span>
                {isActive(link.path) && link.path !== '/booster' && (
                <div className="absolute -top-1 w-1 h-1 bg-neutral-900 rounded-full" />
                )}
            </Link>
            ))}
        </nav>
